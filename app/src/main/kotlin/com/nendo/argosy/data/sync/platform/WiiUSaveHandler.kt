...
@Singleton
class WiiUSaveHandler @Inject constructor(
    @ApplicationContext private val context: Context,
    private val fal: FileAccessLayer,
    private val saveArchiver: SaveArchiver
) : PlatformSaveHandler {
    companion object {
        private const val TAG = "WiiUSaveHandler"
    }
    ...
    override suspend fun prepareForUpload(localPath: String, context: SaveContext): PreparedSave? =
        withContext(Dispatchers.IO) {
            ...
            val outputFile = File(this@WiiUSaveHandler.context.cacheDir, "${saveFolder.name}.zip")
    ...
    override suspend fun extractDownload(tempFile: File, context: SaveContext): ExtractResult =
        withContext(Dispatchers.IO) {
            val targetPath = context.localSavePath ?: run {
                ...
                if (basePath == null) {
                    return@withContext ExtractResult(false, null, "No base path for Wii U saves")
                }
                val titleId = context.titleId
                if (titleId == null) {
                    return@withContext ExtractResult(false, null, "No title ID for Wii U save")
                }
            ...
            if (!success) {
                ...
                return@withContext ExtractResult(false, null, "Failed to extract Wii U save")
            }
    ...
    fun resolveBasePath(config: SavePathConfig, basePathOverride: String?): String? {
        ...
        val resolvedPaths = SavePathRegistry.resolvePath(config, "wiiu", null)
